...
export const myAlcoholicProductUploader = async (req:Request,res:Response,next:NextFunction) => {
    ...
    try {
        ...
        if(uploaded){
            ...
            let Product = new alcoholicBeveragesCatModel({...req.body,productImage:myImage});
            let newProduct = await alcoholicBeveragesCatModel.create(Product);
            if(newProduct){
