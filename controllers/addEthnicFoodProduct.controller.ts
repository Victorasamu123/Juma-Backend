...
export const myEthnicProductUploader = async (req:Request,res:Response,next:NextFunction) => {
    ...
    try {
        ...
        if(uploaded){
            ...
            let Product = new ethnicFoodCatModel({...req.body,productImage:myImage});
            let newProduct = await ethnicFoodCatModel.create(Product);
            if(newProduct){
